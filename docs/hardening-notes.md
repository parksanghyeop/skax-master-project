...
- **[이슈: 변경 추출] 미커밋 테스트 파일이 변경 대상으로 잡힘** (2026-09-14, 시연 장면 2
  실측) — 장면 1의 `cta apply`가 남긴 OrderServiceTest 변경이 커밋되지 않은 채
  `maintain --diff HEAD~1`을 돌리면 `git diff`가 그 테스트 파일까지 포함해, 테스트 메서드
  (`OrderServiceTest#delete_…`)가 변경 심볼로 들어가 의도 분류(ask 항목)·생성 경로로 번졌다.
  해결: 변경 추출은 `src/main/java/` 파일만 대상으로 본다(테스트 트리는 이 도구의 산출물이지
  대응할 동작 변경이 아니다) + 회귀 테스트. 전후: 장면 2 변경 4건(테스트 3건 포함) → 1건

- **[이슈: 운영] 녹화 스크립트 reset이 `.cta/`를 남김** (2026-09-14, 시연 재실행 실측) —
  `.cta/`는 gitignore 대상이라 `git clean -fd`(-x 없음)가 지우지 않는다. 남은 제안이 다음
  녹화의 generate에 "대기 중인 제안"으로 이어 붙어(수정 전 코드에선 실패하는 null 방어 테스트 7개)
  8회 시도 전부 실패 → 사람 확인, resolve는 옛 확인 항목 id를 집었다. 해결: clean_demo가
  `.cta`를 직접 지운다(-x는 examples/demo/.env까지 지우므로 쓰지 않음)

- **[이슈: 프롬프트] 추가 모드 게이트 재시도에서 직전 테스트가 사라짐** (2026-09-14, 시연 장면 1
  실측) — 기존 파일에 추가하는 모드(ADR-0023)는 조각을 **원본 파일** 위에 합친다. 게이트 탈락 후
  재시도 프롬프트의 "[직전 시도에 추가했던 코드]"를 모델이 "이미 파일에 들어간 코드"로 읽고
  부족한 메서드만 내놓아, 회차마다 예외 테스트(delete_missingId)와 정상 경로 테스트가 번갈아
  사라졌다 → 커버리지 게이트 3회 탈락 → "품질 미달". JaCoCo 라인으로 확인: 2회차는 throw 줄만,
  3회차는 deleteById 줄만 미실행. 해결: 프롬프트에 "직전 코드는 아직 파일에 없다, 이번 출력이
  통째로 대체한다"를 명시(합치기 의미는 그대로 — 파일 = 원본 + 이번 조각). 저장된 호출 기록 중
  이 프롬프트를 쓰는 것은 없어 재생성 불필요

- **[이슈: 프롬프트] --impact 파생 건에서 변경된 클래스를 mock해 regression 게이트 탈락** (2026-09-14,
  시연 장면 2 실측) — 호출자(OrderController.get) 테스트가 OrderService를 mock으로 두면 findById의
  null 방어가 실행되지 않아 수정 전 코드에서도 통과한다. 게이트 탈락 사유를 받은 2회차에서 모델이
  mock은 그대로 둔 채 `assertThrows`만 넣어 "nothing was thrown"으로 8회 연속 실패 → 사람 확인
  (같은 실행에서 다른 회차엔 실제 객체를 써서 통과 — 비결정적). 해결: 파생 건 지침서(core/pipeline)와
  regression-test 스킬에 "변경된 클래스는 실제 객체, 그 의존 객체만 mock"을 명시
