...
| `EmbeddingClient` (포트, `llm/embeddings.py`) | `embed(texts: list[str], model: str) -> list[list[float]]` | 자연어 → 벡터(ADR-0026 D3, 판단 메모 검색 전용 — 코드 검색 금지 v4 4.1 ④). 구현: GatewayEmbeddingClient(`/openai/deployments/{deployment}/embeddings`) / RecordingEmbeddingClient(기록: (모델, 글)→벡터) / ReplayEmbeddingClient(재생, 없는 글은 CassetteError). `cosine(a, b)` 순수 파이썬 |
...
| `make_embedding_client` | `(dotenv_path=None) -> (EmbeddingClient, deployment) \| None` | 게이트웨이 주소·키가 없으면 **None**(호출부는 이름 일치 검색으로 폴백). deployment는 `CTA_EMBEDDING_MODEL`(기본 text-embedding-3-small, ADR-0013) |
...
| 판단 메모 `memos.py` | `<프로젝트>/.cta/memos/<시각>-<순번>.json` = `Memo(target, category, decision, note, created_at, situation="", embedding=None)`. 순번은 같은 시각 저장의 덮어쓰기 방지(Windows 시계 해상도). `situation_text(commit_message, target, analysis="")`가 저장·검색 양쪽의 임베딩 원문(자연어만). `find_similar(project, target, query_vector=None)` 하이브리드: 같은 메서드→같은 클래스 최근순을 먼저, 빈 자리는 코사인 ≥ `SIMILARITY_MIN`(0.35) 순으로 최대 3건. 벡터 없으면 이름 일치만. 참고용 — 규칙표 우회 불가 |
