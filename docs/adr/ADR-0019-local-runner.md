...
- 상태: **일부 대체 (2026-09-07) — 결정 1·2·4(기본값 Docker, `--fast`=로컬 실행, graph·demo는 Docker 유지)는 ADR-0022로 바뀌었다.**
  `LocalSandbox`·경로 번역·안전장치 유지(결정 3·5)는 그대로다. 원래 승인 2026-09-06. 사용자 요청 — "Docker로 실행하는 게 너무 오래 걸린다. 빠른 진행을 `--fast`로 넣고 싶다"
