...
2. 저장하는 관계는 **100% 확정되는 것** 3종 + **추정** 1종(호출 관계 CALLS, 확신도 필수 — ADR-0026).
   추정 엣지는 화면·지침서·`--impact` 파생 생성에만 쓰고 조치 결정에는 넣지 않는다
...
| 엣지 | 방향 | 근거 | 예 |
|---|---|---|---|
| DECLARES | 클래스 → 메서드 | 정적 파싱 (소스에 적혀 있음) — 확정 | `Calculator → Calculator#divide` |
| CREATES | 메서드 → 클래스 | 정적 파싱 (`new 클래스(` 구문, 프로젝트 안 클래스만) — 확정 | `CalculatorTest#add_... → Calculator` |
| COVERS | 테스트 클래스 → 메서드 | **JaCoCo 커버리지 실측** (실제 실행 기록) — 확정 | `CalculatorTest → Calculator#add` |
| CALLS | 메서드 → 메서드 | 정적 파싱 **추정** — `confidence`(high/medium)와 호출 줄 `excerpt`를 붙인다 (ADR-0026, `adapters/java/calls.py`). main 트리만 | `OrderService#pay → OrderService#findById [high]` |

CALLS 확신도 규칙 (전부 정규식, LLM 없음):

| 호출 모양 | 확신도 |
|---|---|
| 같은 클래스 안의 `이름(` (수식 없음) | high |
| `x.이름(` 이고 x의 선언 타입(필드·파라미터·지역변수)이 프로젝트 클래스 X, X에 `이름`이 있다 | high |
| `x.이름(` 이고 타입은 못 읽었지만 `이름`을 가진 프로젝트 클래스가 하나뿐 | medium |
| 그 밖 — 타입이 프로젝트 밖(List 등)·이름 중복·생성자·재귀 | 엣지 없음 (없는 것이 틀린 것보다 낫다) |
...
질의 4종이 실제로 답하는 것 (나머지 2종은 후순위 — 안내 문장 반환):
...
| callers | "OrderService#findById를 호출하는 곳 (정적 추정): OrderController#get [high], OrderService#pay [high] …" | `cta maintain`의 영향 범위 줄·작업 지침서(호출자 경유 시나리오), `--impact` 파생 생성 건. **조치 결정에는 쓰지 않는다** |
...
- **CALLS는 추정이다** — 상속·다형성·리플렉션·DI 경유 호출은 잡지 못하고, 변수 이름이 클래스 안에서
  다른 타입으로 두 번 선언되면 나중 것으로 본다. 그래서 확신도를 붙이고, 조치 결정·기존 테스트
  상태에는 넣지 않는다(ADR-0026 D2). 호출자의 테스트가 정말 대상을 실행하면 COVERS가 이미 실측한다
