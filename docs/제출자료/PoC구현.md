...
- **다음 Task 확장 범위**: 워크플로우에 상황별 스킬(테스트 작성 지식 묶음) 붙이기 → 전/후 수치 비교 · 판단 메모 검색 · 멀티모듈 Maven
...
모듈 경계를 건너는 데이터만 모았다. 이 표가 맞으면 다른 진입점을 붙여도 같은 입출력으로 설계할 수 있다.
...
실측 중 만난 문제 12건 가운데 대표 3건이다. 전체 표(프롬프트 3 · 도구 연동 2 · 데이터 3 · 안전장치 2 · 결함 점검 2)는 [핵심구현.md §9](핵심구현.md#9-주요-문제-해결과-기술-리서치--전체-표)에 있다.
...
| `docs/제출자료/테스트및고도화.md` | 테스트·고도화 산출물 — 검출률 평가(로컬 결함 세트), 의도 판단 개선 전/후, 성능·비용, 가드레일 검증, 문제 해결 사례 |
