...
adapters  Java·Maven·git을 아는 쪽      diff→메서드 매핑, 재료 수집, Docker 실행, 게이트 6종
core      언어를 모르는 순수 로직       규칙표, 작성 루프(LangGraph), 도구 6종, 데이터 모델
llm       LLM 호출의 유일한 통로        프롬프트 파일, 게이트웨이, 녹화·재생
...
* **구현 기능:** 5단계 파이프라인(변경 추출 → 의도 분류 → 기존 테스트 실행 → 규칙표 → 작성 루프/사람 확인) +
...
**데이터 흐름 관점** — 무엇이 어디를 거쳐 어디에 남는가

![데이터 흐름 워크플로우](images/workflow-data.png)

파랑 = core / 하늘 = adapters / 보라 = LLM / 주황 = 결정적 안전장치 / 초록 = 보관소(`.cta/`)

* **동작 원리**

  - **`cta generate` — 테스트 생성 (SC-001).** 화면에 네 단계가 순서대로 나온다.

    | 단계 | 하는 일 | 누가 |
    |---|---|---|
    | [1/4] 재료 수집 | 공개 메서드 중 기존 테스트가 참조하지 않는 것을 확인 항목 많은 순으로 N개 선정. 확인 항목(분기·경계값·예외·null) 열거 | 일반 코드 |
    | [2/4] 객체 만드는 법 | 파라미터·의존 타입마다 직접 생성(표준/builder/값 객체) 또는 mock(저장소 인터페이스) 판단 | 일반 코드 |
    | [3/4] 테스트 작성 | 재료를 프롬프트에 실어 LLM이 파일 전체를 작성 → Docker에서 컴파일·방금 만든 테스트만 실행 → 실패하면 직전 코드+실패 로그로 재시도 | LLM + Docker |
    | [4/4] 품질 확인 | 게이트 실측 → 확인 항목 충족 수, 버그 검출력, 기준 낮춤 여부 | 일반 코드 |

    기존 테스트 파일이 있으면 그 파일에 메서드를 추가한다(기존 메서드는 게이트가 보호).
    결과 상태와 종료 코드: 정상 완료 0 / 사람 확인 필요 3 / 품질 미달 2 / 실패 1.

  - **`cta maintain` — 변경 대응 (SC-002 · SC-003).**

    ```
    ① 변경 추출  git diff → 메서드 단위 + 단서(커밋 메시지·이슈 번호·시그니처·줄 수)     일반 코드
    ② 의도 분류  건별 LLM 1회 → 판단·확신도·근거·분석 (주석만 바뀐 심볼은 생략)          LLM
    ③ 기존 테스트  COVERS 실측(또는 참조 파싱)으로 찾아 Docker 실행 → pass/fail/none        일반 코드
    ④ 규칙표     의도 × 테스트 상태 → create_test / no_action / escalate / ask            일반 코드
    ⑤ 처리       create_test → 작성 루프(+regression 게이트) / escalate·ask → 저장하고 멈춤
    ```

    | 의도 | 기존 테스트 | 할 일 |
    |---|---|---|
    | 버그 수정 · 새 기능 | 무관 | 테스트 추가. 버그 수정은 수정 전 코드에서 실패해야 통과(regression) |
    | 리팩터링 | 통과 | 없음 |
    | 리팩터링 | **실패** | 사람 확인 — 저장하고 멈춤 |
    | 리팩터링 | 없음 | 사람에게 질문 |
    | 의미 없는 변경 | 무관 | 없음 (LLM 미호출) |
    | 판단 불확실 | 무관 | 사람에게 질문 |

    "기대값을 자동으로 고친다"는 행이 없다는 점이 이 표의 핵심이다. LLM 분석은 작업 지침서 내용만 채우고 **길은 못 바꾼다**.
    프롬프트는 "category는 작성자의 의도, 동작 보존 여부는 기존 테스트 실행이 판정한다 — 의심 지점은 근거에 적어라".
    자세한 흐름은 `docs/의도분류.md`.

  - **`cta resolve` — 판단 전달 (SC-003 8단계).** 저장된 지점부터 같은 작성 루프로 이어 간다.

    | 옵션 | 뜻 | 에이전트가 하는 일 |
    |---|---|---|
    | `--intended` | 일부러 동작을 바꾼 게 맞다 | 실패한 테스트의 기대값을 새 동작 기준으로 수정 |
    | `--test-issue` | 테스트 쪽 문제다 | 실패한 테스트를 동작 기준으로 다시 작성 |
    | `--proceed` | 계획대로 진행 | 질문 항목에 대해 테스트 생성 |
  ...
  - **작성 루프(LangGraph).** 정보 수집 → 생성 → 컴파일·실행 → 실패 시 실패 로그+직전 코드를 다음 프롬프트에 넣어
    자기 수정(최대 8회, 같은 실패 2회 또는 4회마다 사용자 확인). 게이트 탈락 시 사유를 문장으로 돌려주고 재생성(최대 3회).
...
* **동작 원리**
  ...
  | 구성 요소 | 역할 | 위치 |
  |---|---|---|
  | 변경 추출 | git diff를 메서드 단위로 묶고 단서를 센다 | `adapters/java/changes.py` |
  | 의도 분류 | 변경 한 건당 LLM 1회, JSON으로 판단·확신도·근거 | `llm/intent.py`, `llm/prompts/classify_intent.md` |
  | 규칙표 | 의도 × 테스트 상태 → 할 일. 표 조회뿐 | `core/pipeline/decide.py` |
  | 재료 수집 | 메서드 선정, 확인 항목(분기·경계값·예외·null), 객체 생성법(builder/값 객체/mock), 기존 테스트 파일 | `adapters/java/materials.py` |
  | 작성 루프 | 정보 수집 → 생성 → 컴파일·실행 → 실패 시 재시도, 사용자 개입 지점 | `core/writer_graph.py` |
  | 도구 6종 | inspect_target · query_code_graph · write_test · run_tests · check_quality · report_finding | `core/tools/` |
  | 게이트 6종 | assert · skip · scope · coverage · mutation · regression | `adapters/java/gates.py` 외 |
  | 코드 그래프 | DECLARES · CREATES(정적) · COVERS(JaCoCo 실측). 유사 테스트·검증 테스트 검색 | `graph/`, `cli/graph_access.py` |
  | 보관소 | proposals(제안) · escalations(사람 확인) · memos(판단 메모) | `cli/proposals.py` 외 |
  | 녹화·재생 | 모든 LLM 호출을 JSON으로 저장, 테스트·시연은 재생(비용 0) | `llm/replay.py` |

  - 게이트 6종은 전부 일반 코드다. 테스트를 만든 에이전트는 판정에 관여하지 못한다.

  | 게이트 | 검사 방법 | 잡는 것 |
  |---|---|---|
  | assert | 테스트 메서드 단위 전/후 비교 + 엄격함 점수 | 기존 검증 삭제·완화 (`assertEquals` 4점 → `assertNotNull` 1점) |
  | skip | 어노테이션 카운트 | `@Disabled` 부착 (전체 경로 표기 우회 포함) |
  | scope | 소스 전체 해시 대조 | 허용 목록 밖 수정 (대상 코드 조작) |
  | coverage | JaCoCo 실측 | 대상 라인 80% · 분기 70% 미달 → 확인 항목 충족 수로 표시 |
  | mutation | PIT (복제 pom, 대상 메서드만 집계) | 심은 버그를 못 잡는 빈 테스트 → 버그 검출력 % (전후 비교) |
  | regression | 수정 전 소스로 바꿔 끼워 실행 (git show) | 수정 전 코드에서도 통과하는 재발 방지 테스트 |

  탈락하면 사유를 문장으로 모델에 돌려주고 다시 만든다(최대 3회). 소진하면 "품질 미달"로 사람 확인 제안이 남는다.

  - 그 밖의 장치: 전체 테스트 실행 금지(빈 selector는 어댑터가 거부) / 실행은 네트워크 차단 Docker, 의존성 캐시는
    읽기 전용 마운트 / 소스 트리에 닿는 곳은 `cta apply` 하나 / 실행 도중 예외(시간 초과·Ctrl+C)가 나도 생성물을 되돌린다

* **주요 기술:** Docker(`--network none`, 읽기 전용 마운트), Maven go-offline + 예열, JaCoCo, PIT(+junit5 플러그인), Mockito(대상 앱)

### 2.3 데이터 및 컨텍스트 (RAG & Context)

* **구현 기능:** Neo4j 코드 그래프(실측 연결, 파싱 폴백) + 유사 테스트 본보기 검색 + 판단 메모 + LLM 호출 녹화·재생

* **동작 원리**
  - **코드 그래프에는 확정 관계만** 넣는다: 선언한다(DECLARES) / 만든다(CREATES, 정적 파싱) / 검증한다(COVERS, **JaCoCo 실측**).
    데모 프로젝트 실측: 클래스 13, 메서드 65, 엣지 97. "이 메서드를 검증하는 테스트는?"이 실측 기반이라
    "기존 테스트가 깨졌는가"의 근거가 된다. `generate`는 유사 테스트 본보기를, `maintain`은 검증 테스트를 그래프에서 찾고,
    접속이 안 되면 파싱 폴백으로 내려가며 어느 쪽인지 화면에 표시한다
  - **판단 메모**: `cta resolve`의 결정을 저장하고 같은 메서드·클래스의 사례를 다음 `maintain`의 "참고"로 보여준다 —
    참고일 뿐 규칙표를 우회하지 못한다
  - **녹화·재생**: 모든 LLM 호출은 한 계층만 경유하고 요청·응답(토큰 수 포함)을 JSON으로 저장한다 → `cta demo`·자동 테스트는
...
* **에이전트 동작:**
  1. 변경 추출 — 메서드 2건(`applyDiscount`, `total`)과 단서(fix 커밋 메시지, 이슈 #4821, 연산자 변경, 시그니처 불변 +1/-1)
...
* **에이전트 동작:** 저장된 LLM 호출 기록으로 대표 시나리오를 재생 — 요청 전문이 기록과 같아야 응답을 돌려주므로 결정적
...
- COVERS는 테스트 클래스 단위 실측이다. 메서드 단위 귀속은 비용이 커서 미룬 상태
...
| `docs/contracts.md` | 데이터 모델·포트·게이트 시그니처 |
