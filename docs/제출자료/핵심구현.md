...
adapters  Java·Maven·git을 아는 쪽      diff→메서드 매핑, 소스 파싱, 재료 수집, 테스트 실행, 게이트 6종
...
sandbox   실행 장치 (로컬 기본 / Docker) Docker일 때 네트워크 차단, 마운트 통제
```
...
| ③ 테스트 작성 루프 | 재료 수집 → LLM 생성 → 실행 → 실패하면 실패 로그와 직전 코드로 재시도 | LLM + 실행 장치 | 통과한 테스트 파일 |
...
| [3/4] 테스트 작성 | 재료를 프롬프트에 실어 LLM이 파일 전체를 작성 → 이 PC의 Maven(또는 `--runner docker`)에서 컴파일·방금 만든 테스트만 실행 → 실패하면 재시도 | LLM + 실행 장치 (1.4절) |
...
③ 기존 테스트  COVERS 실측(또는 참조 파싱)으로 찾아 실행 → pass/fail/none        일반 코드 (3.7절)
...
| run | `run_tests`로 지정한 테스트(selector)만 실행 장치에서 실행. 회차 기록(history)에 추가 | route_after_run |
...
의도만으로 할 일이 정해지지 않는다. 대상 메서드를 실제로 실행하는 테스트를 찾아(3.7절) 실행 장치에서 돌려
...
**COVERS 실측 (`adapters/java/coverage.py`)**: 테스트 클래스를 하나씩 실행 장치에서 JaCoCo(커버리지 측정 도구)를 붙여
...
그 밖의 장치: 전체 테스트 실행 금지(실행할 테스트를 지정하지 않으면 거부) / `--runner docker`일 때 `--network none`, 의존성 캐시는 읽기 전용 마운트 /
